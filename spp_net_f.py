    ...
    def inference(self, data, train=True, num_class=10, tp=None):
        if tp is 'spp':
            with tf.name_scope('SPP'):
                ...
                self.conv2 = self._conv_layer(self.pool1, 'conv2', [5, 5, 16, 16])
                ...
                print('numH', numH)
                self.fc7 = self._fc_layer(self.sppool, 'fc6', shape=[numH, 256])
                if train:
                    self.fc7 = tf.nn.dropout(self.fc7, 0.5, seed=SEED)
                self.output = self._fc_layer(self.fc7, 'output', shape=[256,num_class])
                print('inference')
                return self.output
        else:
            with tf.name_scope('cnn'):
                ...
                self.output = self._fc_layer(self.fc1, 'output', shape=[256,num_class])
                print('inference')
                return self.output
